...
func main() {
	...
	if tls != "" && tls != "false" && tls != "no" {
		...
		go nonsecureToSecureRedirector()
	}

	rx.Use( // Logging middleware
...
func nonsecureToSecureRedirector() {
	...
	fx.AddCustomMasterHandler( // TODO - replace mysite with your site below
		func(ctx *fasthttp.RequestCtx) { ctx.Redirect("https://mysite.com", fasthttp.StatusTemporaryRedirect) })
